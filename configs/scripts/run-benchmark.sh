...
# Recovery mechanism differs by architecture, deliberately:
#   - WireGuard: wg0 is a passive kernel interface with no daemon managing
#     its lifecycle. `ip link set down/up` is a faithful simulation of a
#     real network blip.
#   - Tailscale: tailscale0 is actively managed by the tailscaled daemon.
#     Toggling the raw link state externally may not be how the daemon
#     expects to recover. `tailscale down` / `tailscale up` is the
#     architecturally correct recovery primitive — it's what tailscaled
#     itself uses to reconcile state, same as switch.sh already relies on.
#   This asymmetry is itself a real, reportable difference between the two
#   architectures, not an inconsistency in methodology.
# Recovery mechanism differs by architecture, deliberately:
#   - WireGuard: `ip link set down` flushes the wg0-specific route
#     (10.8.0.0/24 dev wg0) that wg-quick installed; `ip link set up` alone
#     does NOT restore it (confirmed by manual testing — traffic silently
#     falls back to the default route via the host's main interface,
#     which cannot actually reach the tunnel subnet). wg-quick is the only
#     thing that re-adds that route, so recovery must go through it.
#   - Tailscale: tailscale0 is actively managed by the tailscaled daemon.
#     `tailscale down` / `tailscale up` is the architecturally correct
#     recovery primitive — it's what tailscaled itself uses to reconcile
#     state, same as switch.sh already relies on.
#   This means BOTH arms now use their VPN's own native restart primitive
#   for recovery, rather than a generic ip-link toggle — which only worked
#   for neither, by coincidence of testing order. Worth a sentence in the
#   report: "simulated recovery" means restarting the VPN's own client
#   process/config, not severing the physical link itself.
...
# Re-poll until the tunnel is reachable again (ping-level — a lighter check
# than the full switch.sh ladder, since the daemon/registration state hasn't
# changed, only the link).
RECOVERY_TIMEOUT=30
RECOVERED="false"
...
# Build JSON payload. NOTE: the backend's own doc comment says null/omitted
# means "not measured this run" — but its actual validator rejects null for
# numeric fields (confirmed: recovery_time_s rejected with
# "`recovery_time_s` must be a number"). Defaulting to 0 here instead of
# null, applied to every field, so any future parse failure doesn't hit the
# same rejection elsewhere. NOTE FOR THE REPORT: a 0 in any of these columns
# may mean "genuinely zero" OR "this run failed to measure it" — they are
# not distinguishable in the data as stored. Worth a caveat in your write-up,
# or revisit this once the backend's validator is fixed to actually accept
# null as documented.
to_json_num() {
    local v="$1"
